...
class WebScraper:
    ...
    async def scrape_company_contacts(self, website: str) -> Dict[str, any]:
        ...
        try:
            ...
            # Wait additional time for Cloudflare challenge to resolve
            ...
            if 'cloudflare' in text_content.lower() and 'verifying you are human' in text_content.lower():
                logger.warning(f"Cloudflare challenge still blocking access to {website}")
                await page.close()
                return {
                    'phone': None,
                    'email': None,
